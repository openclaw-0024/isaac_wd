...
| 内容                                       | 文件                                                                               |
| ---------------------------------------- | -------------------------------------------------------------------------------- |
| 机器人 `ArticulationCfg`、关节名常量、足部/躯干 link 名 | `source/isaac_wd/isaac_wd/robots/fr_robot.py`                                    |
...
`fr_robot.py` 中设置了 `soft_joint_pos_limit_factor=0.9`，Isaac Lab 会在关节软限位上相对 URDF 再收缩，**实际控制/奖励用的有效范围**略小于下表 raw 限位。
...
- 文档与 `fr.urdf`（0408 版本）及 `fr_robot.py` 中的关节列表对齐；若你更新 URDF，请同步更新本文件中的限位表与路径。